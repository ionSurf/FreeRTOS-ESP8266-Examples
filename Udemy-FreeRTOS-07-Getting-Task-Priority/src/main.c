...
void Y_Controller_Task(void *pvParameters) {
  while(1) {
      YLEDProfiler++;
      YTask_Pr  = uxTaskPriorityGet(NULL);
      BTask_Pr  = uxTaskPriorityGet(BTask_H);
...
void setup() {
 ...
 xTaskCreate(&Y_Controller_Task, "YELLOW LED Task", 100,NULL,3,&YTask_H); 
}

void user_init(void) {
    printf("\nUdemy FreeRTOS Course - 07 Getting Task Priority\n");
